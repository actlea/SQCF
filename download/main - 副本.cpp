...
int main(int argc, char* argv[])
{
	...
	if(!LinkSet.count(QueLink)) //判断该链接是否处理过
	{
		...
		//对seedLink的源文件进行处理
		//ofstream onfile;
		//onfile.open(nationalTeam,ios::app);
		...
		while(i<SrcLen)
		{
			...
			//判断该链接是否是我们需要的链接,如果是，从Link中提取出链接存入队列中
			if(JudgeFootballInfo(Link))
			{
				cout << "提取" << QueLink << " 中的链接 " << endl;
				LinkURL=SelectURL(Link);//从带链接的锚文本中提取出链接
				//如果以前没有这个链接，则加入
				...
				if(LinkURL.find("continent")!=string::npos)
				{
					//提取country链接
					
					//GetAllConLink(LinkURL);
				}
				//如果是country,那么进入，下载player链接并提取country信息
				else if(LinkURL.find("country")!=string::npos)
				{
					//提取国家信息
					//GetConInfo(LinkURL);
					//提取包含队员信息的链接
					//GetPlayerLink(LinkURL);
				}
				//如果是player,那么提取player信息
				else if(LinkURL.find("player")!=string::npos)
				{
					//提取队员信息
					//GetPlayerInfo(LinkURL);
				}
				//onfile << LinkURL + "\n" ;
			 }// end if
			Temp = Linkend;
